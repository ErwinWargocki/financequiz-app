...
import 'study_topic_detail.dart';
...
class _StudyTopicsListScreenState extends ConsumerState<StudyTopicsListScreen> {
  ...
  Map<String, List<StudyTopic>> get _grouped {
    ...
    for (final t in _topics) { (map[t.quizCategoryId] ??= []).add(t); }
    return map;
  }
  ...
  Widget build(BuildContext context) {
    ...
    for (final entry in grouped.entries) {
      ...
      for (final topic in entry.value) { items.add(_ListItem.topic(topic)); }
    }
            ...
            child: Padding(
              ...
              child: Container(
                ...
                child: Text(
                  ...
                  style: AppTheme.labelSmall.copyWith(color: color, fontWeight: FontWeight.w700, fontSize: 12),
                ),
              ),
            ),
          ),
          SliverPadding(
            ...
            sliver: SliverList(
              delegate: SliverChildBuilderDelegate(
                (context, i) {
                  final item = items[i];
                  if (item.isHeader) return _FolderHeader(categoryId: item.categoryId!, color: color);
                  final topic = item.topic!;
                  return StudyTopicTile(topic: topic, onTap: () => _openTopic(topic));
                },
  ...
  void _openTopic(StudyTopic topic) {
    ...
    showModalBottomSheet(
      ...
      builder: (_) => StudyTopicDetailSheet(
        topic: topic,
        onTakeQuiz: user != null ? () { Navigator.pop(context); _startQuiz(topic); } : null,
      ),
    );
  }
  ...
  void _startQuiz(StudyTopic topic) {
    ...
    Navigator.pushNamed(context, AppRoutes.quiz, arguments: QuizArgs(category: cat, userId: user!.id!));
  }
}
...
class _ListItem {
  ...
  const _ListItem.header(String id) : isHeader = true, categoryId = id, topic = null;
  const _ListItem.topic(StudyTopic t) : isHeader = false, categoryId = null, topic = t;
}
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          AppSpacing.w10,
          Text(label, style: GoogleFonts.spaceGrotesk(color: p.text, fontSize: 15, fontWeight: FontWeight.w700)),
          AppSpacing.smH,
